...
{
  double *STMtx,*Tinv,sGf,*GfST,dt,*CumNDF,x1,x2,binw,T2,k1,k2,s1,s2,resol;
  long i,j,m,k,Ncells,Nst,Lpdf,Lmax;
    ...
    GfST=mxGetPr(plhs[0]);
    /* resolution must be greater than or equal to smallest bin-size element^-1 */
    resol=10000;
    for (j=0;j<m;j++) {
		      if (resol<pow(10,ceil(-log10(Tinv[j+1]-Tinv[j])+1))) resol=pow(10,ceil(-log10(Tinv[j+1]-Tinv[j])+1));   
		      }
    /*mexPrintf("%e\n",resol);*/

    dt=sGf/resol;
    Lmax=(long)(sqrt(-log(cutoff)*2)*resol-0.5+1.0);
    Lpdf=(long)((resol/10000)*(Tinv[m]-Tinv[0])/dt+1);
    Lpdf=FMIN(Lpdf,Lmax);

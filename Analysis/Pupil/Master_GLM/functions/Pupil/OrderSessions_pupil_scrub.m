function Matrices = OrderSessions_pupil_scrub(PVdirectory, ID)
    create = 0;
    if ~exist(PVdirectory + "/Session_pupil", "dir")
        mkdir(PVdirectory + "/Session_pupil")
    end
    if exist(PVdirectory + "/PVsmall/PVsmall_" + num2str(ID) + ".mat")
        load(PVdirectory + "/PVsmall/PVsmall_" + num2str(ID) + ".mat")
        create = 1;
        fileName = "/Session_pupil/OrderedSessions_pupil_scrub_" + num2str(ID) + "";
    ...
    if create == 1
        ...
        uu = 0;
        for u = 1:Mice
            diam = PVsmall.pupil(u).scrub_trace;
            diam(diam == 0) = NaN;
            if ~isempty(diam) || ~all(isnan(diam))
                raw = isnan(PVsmall.pupil(u).raw_trace);
                for fy = 1:numel(raw)-5
                    win = sum(raw(fy:fy+5));
                    if sum(win) == 6
                        diam(fy:fy+5) = NaN;
%                         coc = coc+1;

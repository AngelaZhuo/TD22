function Matrices = Master_PupilGLM(Matrices, Events, Regions, nboot, Types)
    Bootstrap_K_only;
    Ranges = [[1, Events(3)]; [Events(3), Events(6)]; [Events(6), Events(6) + 60]];
...
%             end
            for bst = 1:nboot
                for neur = 1:Neurons
                    ...
                    Matrix = M(neur, :, :); Matrix = Matrix(:, :, keep);
                    DM = GenXD_Master(TM, Types, Jitter(:, keep)', Ranges);
                    Trials = sum(keep);
